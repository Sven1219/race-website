                    ...
                    <div class="container mt-4">
                        @foreach($racing as $key => $race)
                            <div class="mb-4">
                                ...
                                @endphp
                                <h2>
                                    <button class="toggle-button btn btn-link" data-race-id="{{ $race->id }}" data-race-venue="{{ $venue }}" data-bs-toggle="collapse" data-bs-target="#venue_{{ $key }}" aria-expanded="false" aria-controls="venue_{{ $key }}">
                                ...
                                <div class="collapse" id="venue_{{ $key }}">
                                    <h4 class="total_race" id="{{$race['id']}}"> Total race: {{ $race['sum'] }} </h4>
        ...
        setTimeout(() => {
            ...
            $.ajax({
                ...
                success: function (data) {
                    $('#total_range').html('Total range: ' + data.sum_range);
                }
            })
        }, 1000);
    </script>
    <script>
        ...
        toggleButtons.forEach((button, index) => {
            button.addEventListener('click', () => {
                ...
                const date = urlParams.get('date');
                const to = urlParams.get('to');
                const venue = $(button).attr("data-race-venue");
                const id = $(button).attr("data-race-id");
                $.ajax({
                    url: '{{ route('betting.total_race') }}',
                    type: "GET",
                    async: true,
                    data: {
                        odd_start: odd_start,
                    ...
                    success: function (data) {
                        ...
                        $(selector).html('Total race: ' + data.sum_race);
                    }
                })
