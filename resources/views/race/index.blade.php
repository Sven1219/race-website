...
@php
    ...
    $race_id = isset($_GET['raceID']) ? $_GET['raceID'] : '';
    $meeting_venue = isset($_GET['short_code']) ? $_GET['short_code'] : '';
    $races = \App\Models\Race::whereDate('date', $date)->where('venue', '=', $meeting_venue)->get();
    $today_races = \App\Models\Race::whereDate('date', $date)->get();
@endphp
    ...
    <div class="col-12">
        <div class="d-flex justify-content-between">
            <div class="col col-4 mb-3">
                <p style="font-size: 18px">{{ ucfirst($venue) }} {{ isset($_GET['raceID']) ? $_GET['raceID'] : '' }} ({{ isset($_GET['date']) ? date('m/d', strtotime($_GET['date'])) : '' }})</p>
            </div>
            <div class="col col-4 mb-3">
        ...
        <div class="card">
            <div class="card-header">
                <div class="dropdown">
                    <button class="btn btn-secondary dropdown-toggle btn-lg" type="button" data-bs-toggle="dropdown" aria-expanded="false">{{ $meeting_venue }}</button>
                    <ul class="dropdown-menu">
                        @foreach($today_races as $key => $race)
                            @php
                                $short_code = explode("/", $race->import_url)[2];
                            @endphp
                            @if($race->venue != $meeting_venue)
                                <li><a class="dropdown-item" href="{{url('racing/'.$short_code.'?date='.$race->date.'&raceID=1&short_code='.$race->venue)}}">{{ $race->venue }}</a></li>
                            @endif
                        @endforeach
                    </ul>
                </div>
                <div id="container" class="mr-3">
                        @foreach($races as $key => $raceItem)
                            @php
                                $short_code = explode("/", $raceItem->import_url)[2];
                                $raceData = json_decode($raceItem->race);
                                $output = '';
                            @endphp
                            @if($short_code == $venue && isset($_COOKIE['zone_time'])) 
                                @foreach ($raceData as $raceKey => $raceDataItem)
                                    @php
                                        $segment = explode("/", $raceDataItem->link);
                                        $timestamp = json_decode($raceItem->time)[$raceKey];
                                        $carbonTimestamp = \Carbon\Carbon::parse($timestamp, 'UTC');
                                        $carbonTimestamp->setTimezone($_COOKIE['zone_time']);
                                        $formattedTime = $carbonTimestamp->format('H.i');
                                        if($race_id == $raceDataItem->raceID) {
                                            $output .= '<div><a href="javascript:void(0);" class="btn btn-success btn-lg text-center">' . $formattedTime . '</a></div>';
                                        } else {
                                            $output .= '<div><a href="https://dogpower.dog/racing/' . strtolower($segment[2]) . '?date=' . $raceDataItem->date . '&raceID=' . $raceDataItem->raceID . '&short_code=' . $raceItem->venue . '" class="btn btn-primary btn-lg text-center">' . $formattedTime . '</a></div>';
                                        }
                                    @endphp
                                @endforeach
                            @endif
                ...
                <table id="table_list" class="table align-middle mb-0 card-table" cellspacing="0">
                    <thead>
                        <tr>
                            ...
                            <th>{{ __('Sex') }}</th>
                            <th class="plc">
                                <input type="hidden" id="plc" value="" />
                                <i class="fa fa-sort" style="display:none" aria-hidden="true"></i> {{ __('PLC') }}
                            </th>
                            <th>{{ __('BOX') }}</th>
                            <th>{{ __('WGT') }}</th>
                            <th>{{ __('DIST') }}</th>
                            ...
                            <th>BON</th>
                            <th class="time_ratio">
                                <input type="hidden" id="time_ratio" value="" />
                                <i class="fa fa-sort" style="display:none" aria-hidden="true"></i> Time Ratio
                            </th>
                            <th>Latest</th>
                            <th>1 SEC</th>
                            <th>MGN</th>
...
@push('scripts')
        ...
        $(document).ready(function() {
            var table = $('#table_list')
                ...
                .dataTable({
                    ...
                    serverSide: true,
                    ajax: {
                        url: 'https://dogpower.dog/get-dog-list',
                        data: function(d) {
                            d.venue = '{{ $venue }}',
    ...
    <script>
        ...
        $(function() {
		    ...
		    'Last 90 Days': [moment().subtract(89, 'days').startOf('day'), moment().endOf('day')],
                    'Last 180 Days': [moment().subtract(179, 'days').startOf('day'), moment().endOf('day')],
		    'Last 360 Days': [moment().subtract(359, 'days').startOf('day'), moment().endOf('day')],
    ...
    <script>
        $(document).ready(function() {
            ...
            document.cookie = "zone_time=" + userTimeZone;
    ...
    <script>
        $(document).ready(function() {
            $('.plc').on('click', function() {
                var icon = $(this).find('i');
                icon.show();
                if (icon.hasClass('fa-sort')) {
                    icon.removeClass('fa-sort').addClass('fa-sort-amount-asc');
                    $('#plc').val('ASC');
                } else if (icon.hasClass('fa-sort-amount-asc')) {
                    icon.removeClass('fa-sort-amount-asc').addClass('fa-sort-amount-desc');
                    $('#plc').val('DESC');
                } else {
                    icon.removeClass('fa-sort-amount-desc').addClass('fa-sort');
                    $('#plc').val('')
                    icon.hide();
                }
                $('#time_ratio').val('')
                $('.time_ratio').find('i').removeClass('fa-sort-amount-desc').removeClass('fa-sort-amount-desc').removeClass('fa-sort-amount-asc')
                var oTable = $('#table_list').dataTable();
                oTable.fnDraw(false);
            });

            $('.time_ratio').on('click', function() {
                var icon = $(this).find('i');
                icon.show();
                if (icon.hasClass('fa-sort')) {
                    icon.removeClass('fa-sort').addClass('fa-sort-amount-asc');
                ...
                $('#plc').val('')
                $('.plc').find('i').removeClass('fa-sort-amount-desc').removeClass('fa-sort-amount-desc').removeClass('fa-sort-amount-asc');
                var oTable = $('#table_list').dataTable();
                oTable.fnDraw(false);
            });
        });
